...
freq_temp = 1 # Hz, so stim_dur is 1/freq_temp
stim_dur = 1/freq_temp # in seconds
...
keep_going = True
#phase = 0
#delta_phase = 1 # if phase in [0,1]
while keep_going:
    #grating_S1_T1.phase = np.mod(clock.getTime() / 0.5, frameDur)
    grating_S1_T1.phase = round(np.mod(clock.getTime(), stim_dur)/stim_dur)/2 # need value of 0 or 0.5 for phase depending on whether we are in the
    # first half of the cycle or the second half
