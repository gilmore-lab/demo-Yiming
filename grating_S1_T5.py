...
expName = 'sinusoidal_grating_S1_T5'  # from the Builder filename that created this script
...
grating_S1_T5 = visual.GratingStim(
...
keep_going = True
while keep_going:
    grating_S1_T5.phase = np.mod(clock.getTime() / 0.5, 1) # move 2 cycles per second
    grating_S1_T5.draw()
    win.flip()
    keys = event.getKeys()
    if len(keys) > 0:
