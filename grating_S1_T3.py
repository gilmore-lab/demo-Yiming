...
expName = 'sinusoidal_grating_S1_T3'  # from the Builder filename that created this script
...
if expInfo['frameRate'] != None:
    ...
    frameDur = 1.0 / 8.0  # could not measure, so guess

# create a default keyboard (e.g. to check for escape)
defaultKeyboard = keyboard.Keyboard()

# Initialize components for Routine "trial"
grating_S1_T3 = visual.GratingStim(
...
keep_going = True
while keep_going:
    grating_S1_T3.phase = np.mod(clock.getTime() / 0.5, 1) # move 2 cycles per second
    grating_S1_T3.draw()
    win.flip()
    keys = event.getKeys()
    if len(keys) > 0:
